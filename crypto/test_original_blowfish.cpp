...
namespace crypto_original_blowfish {
    ...
    private:
        ...
        {
            const int algo = GetAlgoCode(cipher);
            const int mode = GCRY_CIPHER_MODE_ECB;  // 原始版本使用ECB模式
            const size_t klen = gcry_cipher_get_algo_keylen(algo);
...
int main() {
    ...
    std::cout << "=== 原始版本BlowFish加密测试 ===" << std::endl;
    ...
    try {
        // 测试原始版本的BlowFish加密
        std::cout << "=== 原始版本BlowFish (ECB模式, 空格填充) ===" << std::endl;
        std::string encrypted_original = crypto_original_blowfish::Encrypt(test_key, test_data);
        std::cout << "加密结果: " << encrypted_original << std::endl;
        std::cout << std::endl;
        
        std::cout << "=== 与非boost版本对比 ===" << std::endl;
        std::cout << "非boost版本 (TwoFish, CBC, PKCS7): Base64:4gAvRkm99lRG1pY9oeU9fB5UT9ow+ftwdraedzc9x2oU5g6noN+0J1aZ6+ArNKf51Wb06TPMJZEan+ip8eSWvsdlLCaOMTqOviSMMRp8X40=" << std::endl;
        std::cout << "原始版本 (BlowFish, ECB, 空格填充): " << encrypted_original << std::endl;
        std::cout << std::endl;
        
        std::cout << "结果一致: " << (encrypted_original == "Base64:4gAvRkm99lRG1pY9oeU9fB5UT9ow+ftwdraedzc9x2oU5g6noN+0J1aZ6+ArNKf51Wb06TPMJZEan+ip8eSWvsdlLCaOMTqOviSMMRp8X40=" ? "✓" : "✗") << std::endl;
